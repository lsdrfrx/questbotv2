...
# async def start_quest(callback: CallbackQuery, state: FSMContext, db: Session):
...
#         f"""
# По поручению {quest.questioner.questioner_name} задаю {len(quest.questions)} вопроса по проекту {quest.quest_name}. На них нужно ответить в течение 60 минут. Обратите внимание: {quest.comment}.
#     """
#     )
...
# async def get_answer(message: Message, state: FSMContext, db: Session, bot: Bot):
...
#             await finish_quest(message, state, db, bot)
#             return
...
# async def finish_quest(message: Message, state: FSMContext, db: Session, bot: Bot):
...
# async def get_next_time(message: Message, state: FSMContext, db: Session, bot: Bot):
#     data = await state.get_data()
#     quest = data["quest"]
...
#     recepients = [quest.questioner.tg_id]

#     if quest.recepients_users is not None:
#         for chatid in quest.recepients_chats:
#             recepients.append(db.query(User).get(chatid).tg_id)

#     if quest.recepients_chats is not None:
#         for chatid in quest.recepients_chats:
#             recepients.append(db.query(Chat).get(chatid).tg_id)

#     for tg_id in set(recepients):
#         await bot.send_message(
#             text=f"{quest.employee.user_name_short} закончил ответ на вопросы по квесту {quest.quest_name}. Ответы на вопросы следующие:",
#             chat_id=tg_id,
#         )
...
#                 )
#         await bot.send_message(
#             text=f"Следующий ответ по квесту {quest.quest_name} от пользователя {quest.employee.user_name_short} ожидается {next_time.strftime("%d.%m.%Y в %H:%M")}",
#             chat_id=tg_id,
#         )


# @router.callback_query(F.data.split(":")[0] == "reschedule_quest")
# async def reschedule_quest(callback: CallbackQuery, state: FSMContext, db: Session):
#     await callback.answer()
#     assert callback.data
#     assert callback.message
#     quest_id = int(callback.data.split(":")[1])
...
#         await bot.send_message(
#             text=f"""
# {quest.employee.user_name_short} перенес квест {quest.quest_name} на {next_time.strftime("%d.%m.%Y в %H:%M")}
...
# async def need_meeting_for_quest(callback: CallbackQuery, db: Session, bot: Bot):
#     await callback.answer()
...
#     if quest.recepients_users is not None:
#         for chatid in quest.recepients_chats:
#             user = db.query(User).get(chatid)
#             assert user
#             recepients.append(user.tg_id)

#     if quest.recepients_chats is not None:
#         for chatid in quest.recepients_chats:
#             chat = db.query(Chat).get(chatid)
#             assert chat
#             recepients.append(chat.tg_id)

#     for tg_id in set(recepients):
#         await bot.send_message(
#             text=f"""
# {quest.employee.user_name_short} просит организовать созвон по квесту {quest.quest_name}!
# """,
#             chat_id=tg_id,
#         )
